...
try:
    ...
    print(f"Warning: Could not create upload directories. Error: {e}")

if os.path.exists(uploads_dir):
    app.mount("/uploads", StaticFiles(directory=uploads_dir), name="uploads")
else:
    print("Warning: Uploads directory does not exist. Skipping StaticFiles mount.")
...
def on_startup():
    ...
    try:
        ...
        print(f"Warning: Could not create tables. Error: {e}")
    ...
    try:
        ...
        if db.query(Alert).first() is None:
            print("No alerts found, generating initial alerts...")
            generate_cluster_alerts(db)
            generate_hotspot_alerts(db)
        else:
            print("Alerts already exist, skipping generation on startup.")
    except Exception as e:
...
def health_check(db: Session = Depends(get_db)):
    ...
    try:
        ...
        case_count, unit_count, accused_count = 0, 0, 0
        db_alive = False
