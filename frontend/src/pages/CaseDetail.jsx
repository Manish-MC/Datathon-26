...
import { ArrowLeft, MapPin, User, AlertCircle, FileText, Calendar, Sparkles, Loader2, RefreshCw, SearchX, AlertTriangle, UserX, UserCheck, Film, FileAudio, File, CheckCircle } from 'lucide-react';
  ...
  return (
    <div className="flex-1 overflow-y-auto">
      <div className="p-8 pb-0">
        ...
        <div className="bg-[#0c1222] border border-slate-800 rounded-xl p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
          <div>
      ...
      <div className="flex border-b border-slate-800 mb-6 sticky top-0 z-20 bg-[#0a0f1d] px-2 md:px-8 overflow-x-auto">
        <button onClick={() => setActiveTab('overview')} className={`px-4 py-3 text-sm font-medium whitespace-nowrap transition-colors ${activeTab === 'overview' ? 'text-blue-400 border-b-2 border-blue-400' : 'text-slate-400 hover:text-slate-200'}`}>Overview</button>
        <button onClick={() => setActiveTab('parties')} className={`px-4 py-3 text-sm font-medium whitespace-nowrap transition-colors ${activeTab === 'parties' ? 'text-blue-400 border-b-2 border-blue-400' : 'text-slate-400 hover:text-slate-200'}`}>Involved Parties</button>
        <button onClick={() => setActiveTab('timeline')} className={`px-4 py-3 text-sm font-medium whitespace-nowrap transition-colors ${activeTab === 'timeline' ? 'text-blue-400 border-b-2 border-blue-400' : 'text-slate-400 hover:text-slate-200'}`}>Timeline</button>
        <button onClick={() => setActiveTab('evidence')} className={`px-4 py-3 text-sm font-medium whitespace-nowrap transition-colors flex items-center space-x-2 ${activeTab === 'evidence' ? 'text-blue-400 border-b-2 border-blue-400' : 'text-slate-400 hover:text-slate-200'}`}>
          <span>Evidence</span>
          {caseData.evidence?.length > 0 && <span className="bg-blue-500/20 text-blue-400 py-0.5 px-2 rounded-full text-[10px]">{caseData.evidence.length}</span>}
        </button>
        <button onClick={() => setActiveTab('similar')} className={`px-4 py-3 text-sm font-medium whitespace-nowrap transition-colors ${activeTab === 'similar' ? 'text-blue-400 border-b-2 border-blue-400' : 'text-slate-400 hover:text-slate-200'}`}>Similar Cases</button>
      </div>
      ...
      <div className="p-8 pt-0">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-[#0f172a] border border-blue-500/30 rounded-xl p-6 relative overflow-hidden">
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  {summary && <span className="text-xs text-slate-500">Generated {new Date(summary.GeneratedAt).toLocaleString()}</span>}
                </div>
                {!summary && !isGeneratingSummary && (
                  <div className="text-center py-4">
                    ...
                    <button onClick={handleGenerateSummary} className="bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium py-2 px-4 rounded-lg flex items-center justify-center space-x-2 mx-auto transition-colors">
                      <Sparkles className="w-4 h-4" />
                      <span>Generate Summary</span>
                    </button>
                  </div>
                )}
                {isGeneratingSummary && <div className="text-center py-6 flex flex-col items-center justify-center space-y-3"><Loader2 className="w-6 h-6 text-blue-500 animate-spin" /><p className="text-blue-400/80 text-sm font-medium animate-pulse">Analyzing facts & generating summary...</p></div>}
                {summary && !isGeneratingSummary && (
                  <div className="space-y-3">
                    <div className="bg-[#090d16]/50 p-4 rounded-lg border border-slate-800/50 relative group">
                      <p className="text-slate-200 text-sm leading-relaxed italic">"{summary.SummaryText}"</p>
                    </div>
                    <div className="flex items-center justify-between mt-4 border-t border-slate-800/60 pt-3">
                      <button onClick={handleRegenerateSummary} className="flex items-center space-x-1.5 text-xs text-blue-400 hover:text-blue-300 transition-colors bg-blue-500/10 hover:bg-blue-500/20 px-3 py-1.5 rounded-lg border border-blue-500/20">
        ...
        {activeTab === 'timeline' && (
          <div className="max-w-4xl">
            <h2 className="text-xl text-white font-light mb-6 flex items-center">
            ...
            <div className="relative border-l border-slate-800 ml-3 space-y-8 pb-8">
              {getTimeline().map((evt, idx) => (
                <div key={idx} className="relative pl-8">
                  ...
                  <div className="bg-[#111726] border border-slate-800 p-4 rounded-xl shadow-sm">
                    <p className="text-xs text-blue-400 mb-1">{new Date(evt.date).toLocaleString()}</p>
          ...
          <div className="max-w-5xl">
            <div className="flex justify-between items-center mb-6">
              ...
              {hasPermission('upload_evidence') && (
                <button onClick={() => navigate('/evidence/submit')} className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium rounded-lg transition-colors">Add Evidence</button>
              )}
            </div>

            {(!caseData.evidence || caseData.evidence.length === 0) ? (
              <div className="bg-[#111726] border border-slate-800 rounded-xl p-8 text-center">
                <FileText className="w-12 h-12 text-slate-600 mx-auto mb-4" />
                <h3 className="text-slate-300 font-medium mb-1">No Evidence Logged</h3>
                <p className="text-slate-500 text-sm">There is no evidence uploaded for this case yet.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {caseData.evidence.map(ev => {
                  ...
                  return (
                    <div key={ev.EvidenceID} className="bg-[#111726] border border-slate-800 rounded-xl overflow-hidden shadow-sm flex flex-col">
                      {isImage ? (
                        <div className="h-40 bg-slate-900 border-b border-slate-800 overflow-hidden relative group">
                          ...
                          <div className="absolute top-2 right-2 bg-black/60 backdrop-blur-md text-white text-[10px] px-2 py-1 rounded">IMAGE</div>
                        </div>
                      ) : (
                        <div className="h-40 bg-slate-900/50 border-b border-slate-800 flex items-center justify-center flex-col text-slate-500">
                      ...
                      <div className="p-4 flex-1 flex flex-col">
                        <div className="flex justify-between items-start mb-2">
                          <h3 className="text-sm font-medium text-slate-200 truncate pr-2" title={ev.OriginalFileName}>{ev.OriginalFileName}</h3>
                          <span className="text-xs text-slate-500 shrink-0">{(ev.FileSizeBytes / (1024*1024)).toFixed(2)} MB</span>
                        </div>
                        ...
                        <div className="mt-auto pt-4 border-t border-slate-800/50 space-y-2">
                          <div className="flex justify-between items-center">
                            ...
                            <span className="text-xs text-slate-300">{ev.uploader_name} ({ev.uploader_rank})</span>
                          </div>
                          
                          <div className="flex justify-between items-center mt-2 border-t border-slate-800/50 pt-2">
                            <span className="text-[10px] text-slate-500 uppercase tracking-wider">Verification</span>
                            {ev.VerificationStatus === 'verified' ? (
                              <span className="text-[10px] text-emerald-400 font-medium flex items-center bg-emerald-500/10 px-2 py-0.5 rounded-full border border-emerald-500/20">
                                <CheckCircle className="w-3 h-3 mr-1" />
                                Verified by {ev.VerifiedByEmployeeID} — {ev.VerifiedByRankName}
                              </span>
                            ) : (
                              <span className="text-[10px] text-amber-400 font-medium flex items-center bg-amber-500/10 px-2 py-0.5 rounded-full border border-amber-500/20">
                          ...
                          <div className="flex justify-between items-center mt-2">
                            ...
                            <span className="text-xs text-blue-400">{new Date(ev.UploadedAt).toLocaleDateString()}</span>
                          </div>
                          ...
                          {ev.links && ev.links.length > 0 && (
                            <div className="mt-3 flex flex-wrap gap-1">
                              {ev.links.map(link => (
                                <span key={link.EvidenceLinkID} className="bg-slate-800 text-slate-300 text-[10px] px-2 py-0.5 rounded-full border border-slate-700 flex items-center">
